...
import { CheckCircle, Clock, Trash2, UserPlus, Tag } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Alert } from "@/pages/Inbox";

interface AlertDetailProps {
  alert: Alert;
  onAction: (action: "do" | "defer" | "delegate" | "delete") => void;
}
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-semibold mb-6">{alert.title}</h1>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-8">
        <Button 
          onClick={() => onAction("do")} 
          className="flex items-center gap-2 bg-white hover:bg-gray-50 text-gray-800 border border-gray-200 rounded-xl shadow-sm px-6"
          variant="outline"
        >
      ...
      {/* Alert description */}
      <div className="bg-gray-50 rounded-2xl p-8 text-gray-700 mb-6">
        {alert.description}
      </div>
    </div>
  );
